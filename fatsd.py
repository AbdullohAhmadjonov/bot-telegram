...
def youtubee(message):
        ...
        yt720 = str(YouTube(message.text).streams.filter(res="720p").first().filesize)
        yt480 = str(YouTube(message.text).streams.filter(res="480p").first().filesize)
        ...
        for link in soup.findAll('a', {'class': 'btn btn-sm btn-outline-success btn-dl'}):
            try:
                bot.send_video(message.chat.id,link['href'],caption='🎥 Bu video @fast_saverbot orqali yuklab olindi 📥')
            except:
        ...
        a = types.InlineKeyboardButton(f"Video📹 Yuklash 720p ⬇️ {yt720[0:2]}.{yt720[2:4]}Mb",url=urls_v[0])
        c = types.InlineKeyboardButton(f"Video📹 Yuklash 480p⬇️ {yt480[0:2]}.{yt480[2:4]} Mb",url=urls_v[1])
